...
describe('u-button', () => {
  ...
  it('should work with `type` prop', async () => {
    ;['default', 'success', 'danger', 'warning', 'info'].forEach((type) => {
      ...
      expect(wrapper.find('.u-button').classes()).toContain(`u-button-${type}`)
      wrapper.unmount()
    })
  })
  ...
  it('should work with `deep` prop', async () => {
    ...
    expect(wrapper.find('.u-button').classes()).toContain(`u-button-deep`)
    wrapper.unmount()
  })
  ...
  it('should work with `text` props', async () => {
    ...
    expect(wrapper.find('.u-button').classes()).toContain('u-button-text')
  })

  it('should work with `prefix` slot', async () => {
    const wrapper = mount(UButton, {
      slots: {
        prefix: () => h(UIcon, null, { default: () => h(IconMessage) }),
      },
    })
  ...
  it('should work with `suffix` slot', async () => {
    const wrapper = mount(UButton, {
      slots: {
        suffix: () => h(UIcon, null, { default: () => h(IconMessage) }),
      },
    })
    expect(wrapper.find('i').classes('u-icon')).toBe(true)
  })
